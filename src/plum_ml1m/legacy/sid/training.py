    ...
    if config.progressive_schedule:
        ...
        for start_epoch, levels in sorted(config.progressive_schedule):
            ...
            if not 1 <= levels <= n_levels:
                raise ValueError(f"progressive_schedule levels must be in [1, {n_levels}].")
...
def _sample_active_levels(
    ...
    raise ValueError("progressive_sampling must be 'deterministic' or 'uniform_prefix'.")
...
def run_advanced_rqvae_experiment(
    *,
    ...
    for epoch in epoch_bar:
        ...
        epoch_record.update(
            {
                ...
                "per_level_unique_fraction_items": sid_eval["per_level_unique_fraction_items"],
        ...
        should_eval = (
            epoch == 1 or epoch % train_config.eval_every == 0 or epoch == train_config.epochs
        )
        if should_eval:
            eval_metrics = {
                **sid_eval,
        ...
        if patience is not None and epoch - best_epoch >= patience:
            stopped_early = True
            stop_reason = f"sid_uniqueness did not improve for {patience} consecutive epochs"
            break
